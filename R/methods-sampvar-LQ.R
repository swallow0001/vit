canvasSampvarLQ <- setRefClass("canvasSampvarLQClass", contains = "canvasPlotClass",
                               methods = list(
    ...
    plotDataStat = function(env, ...) {
        addStatLine(.self, env, fun = function(x) fivenum(x)[2])
    },
...
load_sampvar_LQ <- function(e) {
    ...
    e$c1$viewports <- NULL
    e$c1$image <- NULL
    tmp.canvas <- canvasSampvarLQ$new()
